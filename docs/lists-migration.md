...
## 已知未驗證項目（Phase 3 啟動前要實測）

| 項目 | 影響 | 建議實測時機 |
|---|---|---|
| `list({ CreatedAt: 'YYYY-MM-DDTHH:mm:ssZ' })` 用 `eq` + 單引號當 string compare 能否 work | Phase 3 shadow-read 若需用 dateTime filter（例如「最近 7 天 feedback」），現有 list 實作可能要加型別分支 | Phase 3 啟動前 |
| `list({ ClosedAt: null })` 查空 dateTime 是否能 work | 同上，dual-write diff 邏輯可能需要區分「有完成日期 vs 沒有」 | Phase 3 啟動前 |
| graphListsDb 對 `graphDb._getAccessToken()` 的依賴 | 重構 graphDb 時若改 method 名稱會打掉 listsDb；要列為 public contract | Phase 1 結案前在 graphDb.js 加 comment |

---

## Milestone 進度（Phase 1）

- [x] **Milestone 1**：`graphListsDb.js` 5 個 feedback CRUD methods（2026-05-26 完成）
- [x] **Milestone 2**：`toListFields` / `fromListFields` 轉換層 + round-trip 17/17 ✅ + 409 ConflictError ✅（2026-05-26 完成）
- [ ] **Milestone 3**：`config.js` FEATURE_FLAGS + `dbAdapter.js` 路由包裝
...
- **⚠️ 毫秒會被吃掉（lossy）**：實測寫入 `2026-05-08T03:33:28.859Z`，讀回變成 `2026-05-08T03:33:28Z`。
  ...
  - **🟢 Milestone 2 round-trip finding (2026-05-26)**：baseline 那筆 FB-20260508-113328-DYUP 實測 17 欄全 ✅，**包含 dateTime**。代表該筆既有 JSON 資料本來就不含毫秒（可能歷史上某次寫入路徑就沒帶 ms）。但這**不代表所有 path 都沒毫秒**——Phase 2 dual-write 若有新 code 路徑用 `new Date().toISOString()` 寫入，仍會踩到截斷。Phase 3 diff 邏輯一定要 normalize dateTime 到秒精度才比對。
...
- **錯的 etag 會收到 HTTP 409 Conflict**（Milestone 2 實測確認，Phase 0.4 Test 5 看到的 412 也可能出現，視 Graph API 路徑而定）：
  ```json
  {
    "error": {
  ...
  ```
  Phase 1 catch error 判斷條件：`e instanceof ConflictError`（最可靠）或 `e.status === 409 || e.status === 412`。
- **409/412 後 item 是真的沒被改的**（實測確認）：拿著舊 etag 試圖寫入 `WILL_NOT_BE_SAVED`，409 後 GET 確認 FbTitle 仍是原值 `TEST`，**SharePoint 是 atomic reject，不會偷偷改**。
...
### Phase 1 新增 Gotchas（2026-05-26）

- **#10 409 vs 412 for etag 衝突**：格式無效的 etag 字串回 409 Conflict；格式正確但過時的 etag 才回 412 Precondition Failed。`_req` 需同時攔截兩種 → throw `ConflictError`。

- **#11 shadow write helpers 同步 throw 問題**：`graphListsDb.feedback.add(...)` 若 `graphListsDb` 未定義，`.feedback` 存取會同步 throw TypeError，promise chain 的 `.catch()` 攔不到。解法：helper 開頭加 `typeof graphListsDb === 'undefined'` 檢查 + 整個 try/catch 包住，確保 shadow write 失敗不會污染主流程。

- **#12 `toListFields` caller 責任**：`feedback_items[docId]` 的 inner object 沒有 `id` 欄位（id 是 outer key）。呼叫前必須補上 `id`：`toListFields({ ...item, id: docId })`，否則 List 的 `Title`（FeedbackId）欄位是空字串。

- **#13 `graphListsDb` 依賴 `graphDb._getAccessToken()`（public method）**：重構 `graphDb.js` 時請保留此 method 為 public，或抽出共用 utility 並更新 `graphListsDb.js` 的引用。
...
- [x] **Phase 1**：寫 `graphListsDb.js` 的 feedback CRUD methods（✅ 2026-05-26 完成，見下方 Phase 1 結案紀錄）
- [ ] **Phase 2**：啟動 dual-write（採用「設計 C：透明寫入 + 顯式驗證讀取」，詳見下方「Phase 2 設計」章節）
  - [x] 2.1 加 feature flag `DUAL_WRITE_FEEDBACK` / `SHOW_DEV_PANEL` / `PRIMARY_FEEDBACK`（已在 config.js 完成）
  - [x] 2.2 shadow write 邏輯（已實作在 `dbAdapter.setDoc/updateDoc/deleteDoc`，非 fbSubmitFeedback，見 Phase 1 retrospective）
  - [ ] 2.3 加開發者驗證區 UI（摺疊式 details）+ 把 `graphListsDb.js` 加入 `index.html`
  - [ ] 2.4 實作 `fbCompareWithList()` 並排 diff 視覺化
  - [ ] 2.5 實作 `fbShowDualWriteLog()` 顯示最近寫入結果（`dbAdapter.getDualWriteLog()` 已備好）
...
**Phase 1 全部 4 個 Milestone 完成**：

- [x] **Milestone 1**：`graphListsDb.js` 純 List CRUD（5 methods：add / get / list / update / delete）
- [x] **Milestone 2**：`toListFields` / `fromListFields` JSON↔List 轉換層，17 欄 round-trip 驗證通過
- [x] **Milestone 3**：`config.js` FEATURE_FLAGS + `dbAdapter.js` feedback 路由層，dual-write 實測兩邊同步寫入
- [x] **Milestone 4**：文件更新 + full smoke test
...
1. **shadow write 位置**
   - doc 設計（Phase 2 設計章節）：shadow write 在 `fbSubmitFeedback()` 內部
   - 實際實作：shadow write 在 `dbAdapter.setDoc/updateDoc/deleteDoc`
   - 理由：放在 dbAdapter 層更透明，不侵入 app 邏輯，也不需要修改 index.html 的 fbSubmitFeedback
   - 影響：Phase 2 的 2.2 實際上已完成，但 UI dev panel（2.3–2.6）仍需加入 index.html

2. **dbAdapter return shape 改動**
   - Milestone 1 文件寫 `add()` 回 `{ id, etag, raw }`
   - Milestone 2 實作改成 `{ spItemId, etag, data }`（data 是 fromListFields 還原的 JSON 形狀）
   - 原因：統一所有 CRUD 的 return shape，data 已轉換好方便 caller 直接用

#### 新發現的 Gotchas（補入本文件 gotcha 列表）

10. **409 vs 412 for etag 衝突**：SharePoint 對格式完全無效的 etag 字串（如 `'BAD_ETAG'`）回 409 Conflict；對格式正確但過時的 etag 才回 412 Precondition Failed。`_req` 需同時攔截兩種 status → throw `ConflictError`。

11. **shadow write helpers 需 try/catch**：`graphListsDb.feedback.add(...)` 若 `graphListsDb` 未定義，`graphListsDb.feedback` 會同步 throw TypeError，`.catch()` 攔不到，錯誤會穿透到主流程。解法：helper 開頭加 `typeof graphListsDb === 'undefined'` 檢查 + 整個 try/catch 包住。

12. **`toListFields` caller 責任**：`feedback_items[docId]` 的 inner object 沒有 `id` 欄位（id 是 outer key）。呼叫 `toListFields` 前必須補上：`toListFields({ ...item, id: docId })`。否則 List 的 `Title`（FeedbackId）欄位會是空字串。

13. **`graphListsDb` 依賴 `graphDb._getAccessToken()`**：`graphListsDb.js` 重用 `window.graphDb._getAccessToken()` 取 MSAL token，不另起 MSAL 實例。若未來重構 `graphDb.js`，請保留 `_getAccessToken` 為 public method，或將其抽出為共用 utility 再更新 `graphListsDb.js` 的引用。

### Phase 2 啟動條件

以下全部符合才進 Phase 2：

- [ ] 在 `index.html` 加入 `<script src="graphListsDb.js"></script>`（Phase 1 為避免修改 index.html 暫不加，Phase 2 必須加）
- [ ] 確認工具頁 `window.graphListsDb` 存在（不需透過 Snippet 注入）
- [ ] `window.FEATURE_FLAGS.DUAL_WRITE_FEEDBACK = true` 下送出一筆回饋，JSON 和 List 兩邊均有資料（已在 Phase 1 Milestone 3 以 Snippet 手動驗證）
- [ ] `dbAdapter.getDualWriteLog()` 回傳 `{ ok: true }` 無錯誤
- [ ] 決定是否先跑 `fbForceListPush()` 補齊歷史 feedback（Phase 1 啟動前只有 baseline item FB-20260508-113328-DYUP 在 List 上）
